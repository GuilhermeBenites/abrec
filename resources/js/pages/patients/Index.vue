...
const filterHealthIndicators = ref<string[]>(
    Array.isArray(props.filters?.health_indicators)
        ? [...props.filters.health_indicators]
        : [],
);
...
watch(
    ...
    (newFilters) => {
        ...
        filterHealthIndicators.value = Array.isArray(newFilters?.health_indicators)
            ? [...newFilters.health_indicators]
            : [];
    },
    { deep: true },
);
...
function applyFilters() {
    ...
    if (filterHealthIndicators.value.length > 0) {
        query.health_indicators = filterHealthIndicators.value;
    }
...
function exportUrl(): string {
    ...
    if (filterCpf.value) {
        params.set('cpf', filterCpf.value);
    }
    for (const indicator of filterHealthIndicators.value) {
        params.append('health_indicators[]', indicator);
    }
    const queryString = params.toString();

    return exportMethod().url + (queryString ? `?${queryString}` : '');
}

function toggleHealthIndicator(key: string) {
    const idx = filterHealthIndicators.value.indexOf(key);
    if (idx >= 0) {
        filterHealthIndicators.value = filterHealthIndicators.value.filter(
            (k) => k !== key,
        );
    } else {
        filterHealthIndicators.value = [...filterHealthIndicators.value, key];
    }
    applyFilters();
}
...
const healthIndicatorOptions = [
    { key: 'diabetic', label: 'Diabético', color: 'blue' },
    { key: 'hypertensive', label: 'Hipertenso', color: 'red' },
    { key: 'renal', label: 'Problema Renal', color: 'orange' },
    { key: 'obesity', label: 'Obesidade', color: 'green' },
];
...
function openDeleteModal(patient: { id: number; name: string }) {
...
function confirmDelete() {
    ...
    router.delete(destroy.url({ patient: patientId }), {
        preserveScroll: true,
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex flex-col gap-8 py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-100 shadow-sm">
                ...
                <div class="border-b border-gray-100 p-6">
                    <div class="grid grid-cols-1 items-end gap-4 md:grid-cols-4">
                        <div class="md:col-span-1">
                            <label class="mb-1 block text-xs font-medium text-gray-700" for="search-name">
                                Nome do Paciente
                            </label>
                            <div class="relative">
                                ...
                                <input id="search-name" v-model="filterName" type="text"
                                    placeholder="Buscar por nome..."
                                    class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 pl-10 text-sm text-gray-900 placeholder-gray-400 focus:border-primary focus:ring-primary"
                                    @input="debouncedApplyFilters" @keydown.enter="applyFilters" />
                            </div>
                        </div>
                        <div class="md:col-span-1">
                            <label class="mb-1 block text-xs font-medium text-gray-700" for="search-cpf">
                                CPF
                            </label>
                            <input id="search-cpf" v-model="filterCpf" type="text" placeholder="000.000.000-00"
                                class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-4 text-sm text-gray-900 placeholder-gray-400 focus:border-primary focus:ring-primary"
                                @input="debouncedApplyFilters" @keydown.enter="applyFilters" />
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <div class="flex flex-wrap gap-3">
                                <label v-for="opt in healthIndicatorOptions" :key="opt.key"
                                    class="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg border px-3 transition-colors"
                                    :class="filterHealthIndicators.includes(opt.key)
                                        ? healthIndicatorBgClasses[opt.color] ?? healthIndicatorBgClasses.gray
                                        : 'border-gray-200 bg-gray-50 hover:bg-gray-100'">
                                    <input type="checkbox" :value="opt.key"
                                        :checked="filterHealthIndicators.includes(opt.key)" class="sr-only"
                                        @change="toggleHealthIndicator(opt.key)">
                                    <component :is="healthIndicatorIcons[opt.key]" class="size-4 shrink-0" />
                        ...
                        <div>
                            <nav aria-label="Pagination" class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                                <button type="button"
                                    ...
                                    :disabled="patients.current_page === 1" :aria-label="'Anterior'"
                                    @click="goToPage(patients.links[0]?.url)">
                                    <ChevronLeft class="size-5" />
                                </button>
                                <template v-for="(link, idx) in patients.links.slice(1, -1)" :key="idx">
                                    <span v-if="!link.url"
                                ...
                                <button type="button"
                                    ...
                                    :disabled="patients.current_page === patients.last_page" :aria-label="'Próximo'"
                ...
                <div v-else-if="patients.total > 0" class="border-t border-gray-200 px-4 py-3 sm:px-6">
                    <p class="text-sm text-gray-700">
                        Mostrando
                        <span class="font-medium text-gray-900">
                            {{ patients.from }}
            ...
            <Dialog v-model:open="isDeleteModalOpen"
                @update:open="(v) => { if (v === false) patientToDelete.value = null }">
